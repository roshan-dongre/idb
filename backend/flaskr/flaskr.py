...
@app.route('/api', methods=['GET']) #, subdomain="api"
def get_info():
    return redirect("https://roshan-dongre.gitbooks.io/api/")

@app.route('/api/states', methods=['GET']) #, subdomain="api"
def get_states():
    return jsonify({'states': ast.literal_eval(str(State.query.all()))})

@app.route('/api/criminals', methods=['GET']) #, subdomain="api"
def get_criminals():
    return jsonify({'criminals': ast.literal_eval(str(Criminal.query.all()))})

@app.route('/api/crimes', methods=['GET']) #, subdomain="api"
def get_crimes():
